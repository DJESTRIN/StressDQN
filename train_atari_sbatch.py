...
if __name__ == '__main__':
    ...
    os.mkdir(f'seed{hyper_params["seed"]}SBATCH')
    ...
    env = gym.wrappers.Monitor(
        env, f'seed{hyper_params["seed"]}SBATCH/video/', video_callable=lambda episode_id: episode_id % 50 == 0, force=True)
    ...
    for t in range(hyper_params["num-steps"]):
        ...
        if done and hyper_params["print-freq"] is not None and len(episode_rewards) % hyper_params[
            ...
            print("********************************************************")
            torch.save(agent.policy_network.state_dict(), f'seed{hyper_params["seed"]}SBATCH/checkpoint.pth')
            np.savetxt(f'seed{hyper_params["seed"]}SBATCH/rewards_per_episode_seed_{hyper_params["seed"]}.csv', episode_rewards,
